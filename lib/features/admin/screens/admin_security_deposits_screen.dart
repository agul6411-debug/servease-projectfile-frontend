...
import 'package:frontfile_servease/features/admin/services/admin_security_service.dart';
...
class AdminSecurityDepositsScreen extends StatefulWidget {
  const AdminSecurityDepositsScreen({super.key});

  @override
  State<AdminSecurityDepositsScreen> createState() =>
      _AdminSecurityDepositsScreenState();
}

class _AdminSecurityDepositsScreenState
    extends State<AdminSecurityDepositsScreen>
  ...
  Future<void> _load() async {
    ...
    final data = await AdminSecurityService.fetchAll();
    setState(() {
      _pending = data.where((d) => d['status'] == 'submitted').toList();
      _history = data.where((d) => d['status'] != 'submitted').toList();
      _isLoading = false;
    });
  }

  Future<void> _verify(int id) async {
    final success = await AdminSecurityService.verify(id);
    _load();
    _showSnack(
      success ? 'Security deposit verified ✓' : 'Failed',
      success ? Colors.green : Colors.red,
    );
  }

  Future<void> _reject(int id) async {
    final success = await AdminSecurityService.reject(id);
    _load();
    _showSnack(
      success ? 'Security deposit rejected' : 'Failed',
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              showActions ? 'No pending deposits' : 'No history yet',
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(10),
                    child: Image.network(
                      '${AppConfig.baseUrl}/uploads/security/${data['screenshot']}',
                      fit: BoxFit.cover,
    ...
    showDialog(
      ...
      builder: (_) => Dialog(
        child: ClipRRect(
          borderRadius: BorderRadius.circular(12),
          child: Image.network(
            '${AppConfig.baseUrl}/uploads/security/$filename',
            fit: BoxFit.contain,
          ),
        ),
      ),
