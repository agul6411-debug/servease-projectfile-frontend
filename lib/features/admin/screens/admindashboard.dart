...
import 'package:frontfile_servease/features/admin/services/adminservice.dart';
...
class _AdminDashboardState extends State<AdminDashboard> {
  ...
  // ── Platform Stats Section (replaces fake health bars) ──────────
  Widget _buildHealthSection() {
    if (dashboardData == null) return const SizedBox();

    final totalUsers = (dashboardData!.totalUsers ?? 0).toDouble();
    final totalProviders = (dashboardData!.totalProviders ?? 0).toDouble();
    final totalBookings = (dashboardData!.totalBookings ?? 0).toDouble();
    final resolvedComplaints = (dashboardData!.resolvedComplaints ?? 0)
        .toDouble();
    final totalComplaints = (dashboardData!.totalComplaints ?? 1).toDouble();

    final providerRatio = totalUsers > 0
        ? (totalProviders / totalUsers).clamp(0.0, 1.0)
        : 0.0;
    final bookingRatio = totalUsers > 0
        ? (totalBookings / (totalUsers * 2)).clamp(0.0, 1.0)
        : 0.0;
    final resolutionRatio = totalComplaints > 0
        ? (resolvedComplaints / totalComplaints).clamp(0.0, 1.0)
        : 1.0;
            ...
            child: Column(
              children: [
                _healthRow(
                  ...
                  providerRatio,
                  AppColors.success,
                  '${totalProviders.toInt()} of ${totalUsers.toInt()} users',
                ),
                ...
                _healthRow(
                  ...
                  '${totalBookings.toInt()} total bookings',
                ),
                const SizedBox(height: 14),
                _healthRow(
                  ...
                  '${resolvedComplaints.toInt()} of ${totalComplaints.toInt()} resolved',
                ),
              ],
            ),
          ),
  ...
  Widget _healthRow(String label, double value, Color color, String subtitle) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
